...
import { AwsProfileStatusBar } from "./contributes/statusBar";
import { fetchEnvFromKube } from "./contributes/commands";
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  if (!prerequisites.allPresent) {
    ...
    logger.info(
      "Extension activation completed with errors (prerequisites not met)"
    );
    return;
  }

  logger.info(
    "All prerequisites satisfied, initializing extension features..."
  );
  ...
  const fetchEnvCmd = vscode.commands.registerCommand(
    ...
    async (uri?: vscode.Uri) => {
      await fetchEnvFromKube(prerequisites);
    }
  );
  ...
  vscode.window.showInformationMessage(
    "AWS & Kubernetes Utilities extension is ready!"
  );
}
