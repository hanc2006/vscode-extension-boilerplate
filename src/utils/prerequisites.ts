...
import { isWorkspaceValid, getWorkspaceRoot, getServiceName } from './workspace';
...
export async function checkPrerequisites(): Promise<PrerequisiteCheckResult> {
  ...
  if (!serviceName) {
    result.errorMessage = 'Failed to read package.json or package.json does not have a valid "name" property.';
    logger.error(result.errorMessage);
    return result;
  }
  ...
  if (result.allPresent) {
    ...
    logger.info(`Prerequisites check failed. Missing binaries: ${result.missingBinaries.join(', ')}`);
  }
